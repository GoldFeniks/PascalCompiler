    ...
    namespace syntax_analyzer {
        ...
        namespace operations {
            ...
            public:
                ...
                template<typename T>
                unsupported_operands_types(const T& left, const T& right, const tokenizer::token::sub_types type) {
                    message_ = str(boost::format("Unsupported operands types \"%1%\", \"%2%\" for %3%")
                        % left->name() % right->name() % tokenizer::token::sub_types_strings[static_cast<unsigned int>(type)]);
                }

                template<typename T>
                unsupported_operands_types(const T& left, const tokenizer::token::sub_types type) {
                    message_ = str(boost::format("Unsupported operand types \"%1%\" for %2%")
                        % left->name() % tokenizer::token::sub_types_strings[static_cast<unsigned int>(type)]);
                }
            ...
            inline const type_p& get_type_for_operands(type_p left, type_p right);
